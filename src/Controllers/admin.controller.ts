...
const adminController = {
  ...
  addModels: catchError(async (req: Request, res: Response) => {
    const { name, gender, language, elevenLabsId, accent, thumbnail } =
      req.body;
    const validator = new Validator();
    if (!name || !gender || !language || !elevenLabsId) {
      validator.RequestBodyValidator.validateRequestBody(
        ...Object.keys(req.body)
      );
    }
    const audioModel = await audioModelRepository.create(req.body);
    res
      .status(201)
  ...
  getAllModels: catchError(async (req: Request, res: Response) => {
    const models = await audioModelRepository.findAll();
    res
      .status(200)
