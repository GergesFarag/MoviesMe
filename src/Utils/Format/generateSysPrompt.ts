export const generateSysPrompt = (
  ...
  return `You are an expert cinematic scene generator focused on creating EXACTLY ${scenesNumber} sequential cinematic scenes from any valid story input. 
...
${storyGenre ? `- Enforce genre conformity: All scenes must follow "${storyGenre}" conventions including mood, archetypes, and setting.` : ``}
${storyLocation ? `- Enforce location consistency: Set all scenes in or around "${storyLocation}" with authentic geographic, architectural, and cultural details.` : ``}

OUTPUT FORMAT:
{
  "title": "${storyTitle ? storyTitle.replace(/"/g, '\\"') : "Generated Story Title (Max 60 Chars)"}",
  "scenes": [
    {
      "sceneNumber": 1,${doNarration ? '\n      "narration": "Concise voiceover text here",' : ""}
